...
{
    ui->label_WMC->clear();//очистка при повторном нажатии
    QStringList lst = code.split(QRegExp("class "),  QString::SkipEmptyParts);
    int s, k=lst.count()-1;
    for(int i = lst.count()-1; i >= 0; --i)
    {
        ...
        for(int j = list.count()-1; j >= 0; --j)
        {
            ...
            if ((ite.count("}")>0)&&ite.count("}")>=ite.count("{"))
                s=s+ite.count("}");
        }
        //if (s>0)
       //     s--;
        ui->label_WMC->setText( ui->label_WMC->text()+"class "+QString::number(k--)+": "+QString::number(s)+", ");
    }
}
